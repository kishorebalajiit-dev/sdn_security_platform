...
export function TopHeader({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  return (
    <header
      ...
      style={{
        background: "linear-gradient(180deg, rgba(0,10,0,0.95), rgba(0,6,0,0.9))",
    ...
    >
      ...
      <div className="top-header__title-block">
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
          ...
          <span style={{ fontSize: "10px", letterSpacing: "0.18em", textTransform: "uppercase", color: "#00FF41", fontWeight: 700 }}>Live defense grid</span>
        </div>
        ...
        <p style={{ fontSize: "12px", color: "#86EFAC", marginTop: "3px" }}>{subtitle}</p>
      </div>
      ...
      <div className="top-header__search-wrap" style={{ position: "relative" }} ref={searchRef}>
        {searchOpen ? (
          <div style={{ position: "relative" }}>
            <div className="top-header__search-input" style={{ display: "flex", alignItems: "center", gap: "8px", background: "rgba(0,8,0,0.8)", border: "1px solid rgba(0,255,65,0.32)", borderRadius: "10px", padding: "9px 14px", boxShadow: "0 0 24px rgba(0,255,65,0.12)" }}>
              <Search size={13} style={{ color: "#00FF41", flexShrink: 0, filter: "drop-shadow(0 0 8px rgba(0,255,65,0.6))" }} />
              <input
                ref={searchInputRef}
                value={searchVal}
            ...
            {searchVal.trim() && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, right: 0, background: "rgba(8,11,26,0.96)", border: "1px solid rgba(0,255,65,0.24)", borderRadius: "14px", overflow: "hidden", zIndex: 200, boxShadow: "0 18px 50px rgba(0,0,0,0.55), 0 0 24px rgba(0,255,65,0.12)" }}>
                {searchResults.length === 0 ? (
                  ...
                  searchResults.map((r) => (
                    <button
                      ...
                      style={{ width: "100%", padding: "12px 16px", background: "none", border: "none", borderBottom: "1px solid rgba(0,255,65,0.06)", cursor: "pointer", textAlign: "left", display: "flex", flexDirection: "column", gap: "2px" }}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(0,255,65,0.08)"; }}
          ...
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: "8px", background: "rgba(17,24,39,0.62)", border: "1px solid rgba(0,255,65,0.18)", borderRadius: "14px", padding: "9px 16px", color: "#94A3B8", cursor: "pointer", fontSize: "12px", transition: "all 0.15s", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.05)" }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(0,255,65,0.42)"; (e.currentTarget as HTMLButtonElement).style.color = "#F8FAFC"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 20px rgba(0,255,65,0.15)"; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(0,255,65,0.18)"; (e.currentTarget as HTMLButtonElement).style.color = "#94A3B8"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "inset 0 1px 0 rgba(255,255,255,0.05)"; }}
          >
            <Search size={13} />
            <span>Search...</span>
            <span style={{ fontSize: "10px", background: "rgba(0,255,65,0.12)", border: "1px solid rgba(0,255,65,0.16)", borderRadius: "999px", padding: "1px 6px" }}>⌘K</span>
          </button>
        )}
      </div>

      <div className="cyber-pill top-header__status-pill" style={{ display: "flex", alignItems: "center", gap: "8px", background: "rgba(0,255,65,0.08)", border: "1px solid rgba(0,255,65,0.22)", borderRadius: "999px", padding: "7px 14px" }}>
      ...
      <div style={{ position: "relative" }} data-topheader-notifications>
        <button
          ...
          style={{ position: "relative", width: "40px", height: "40px", borderRadius: "14px", background: notifOpen ? "rgba(0,255,65,0.18)" : "rgba(17,24,39,0.68)", border: "1px solid rgba(0,255,65,0.18)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#C4B5FD", transition: "all 0.15s" }}
        >
          <Bell size={15} />
          {unreadCount > 0 && (
        ...
        {notifOpen && (
          <div style={{ position: "absolute", top: "calc(100% + 10px)", right: "0", width: "340px", background: "rgba(8,11,26,0.96)", border: "1px solid rgba(0,255,65,0.24)", borderRadius: "16px", boxShadow: "0 20px 50px rgba(0,0,0,0.55), 0 0 24px rgba(0,255,65,0.12)", overflow: "hidden", zIndex: 100 }}>
            <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(0,255,65,0.12)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "13px", fontWeight: 600, color: "#F8FAFC" }}>Notifications</span>
              <button onClick={markNotificationsRead} style={{ fontSize: "10px", color: "#00FF41", cursor: "pointer", background: "none", border: "none" }}>Mark all read</button>
            </div>
            {notifications.map((n) => (
              <div key={n.id} style={{ padding: "12px 16px", borderBottom: "1px solid rgba(0,255,65,0.06)", display: "flex", gap: "10px", alignItems: "flex-start", background: n.unread ? "rgba(0,255,65,0.05)" : "transparent" }}>
            ...
            <div style={{ padding: "10px 16px", textAlign: "center" }}>
              <button onClick={() => { navigate("/alerts"); setNotifOpen(false); }} style={{ fontSize: "11px", color: "#00FF41", cursor: "pointer", background: "none", border: "none" }}>View all alerts →</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ position: "relative" }} data-topheader-profile>
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: "10px", background: profileOpen ? "rgba(0,255,65,0.14)" : "rgba(17,24,39,0.68)", border: "1px solid rgba(0,255,65,0.18)", borderRadius: "14px", padding: "6px 12px 6px 6px", cursor: "pointer", transition: "all 0.15s" }}
        >
          <div style={{ width: "30px", height: "30px", borderRadius: "10px", background: "linear-gradient(135deg, #00FF41, #00CC33)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 800, color: "#000", flexShrink: 0, boxShadow: "0 0 18px rgba(0,255,65,0.3)" }}>
            {user?.initials ?? "?"}
          </div>
        ...
        {profileOpen && (
          <div style={{ position: "absolute", top: "calc(100% + 10px)", right: "0", width: "220px", background: "rgba(8,11,26,0.96)", border: "1px solid rgba(0,255,65,0.24)", borderRadius: "16px", boxShadow: "0 20px 50px rgba(0,0,0,0.55), 0 0 24px rgba(0,255,65,0.12)", overflow: "hidden", zIndex: 100 }}>
            <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(0,255,65,0.12)" }}>
